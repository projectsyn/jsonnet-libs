...
local teamReceiverFormats(adParams) = std.get(adParams, 'team_receiver_formats', [ adParams.team_receiver_format ]);
...
local teamBasedRouting(adParams, nullReceiver) =
  std.flattenArrays(std.map(
    function(k) [ {
      ...
      continue: true,
    } for format in teamReceiverFormats(adParams) ],
 ...
 *   `prepend_routes`, `append_routes`, `team_receiver_formats`, and
 ...
 *
 *   Field `team_receiver_formats` is used to render Alertmanager receiver
 *   names for each discovered team. The library expects that this field
 *   contains a list where each element contains exactly one '%s' which is
 *   substituted with the target team's name. Users of the library must ensure
 *   that matching receivers are defined in `amConfig.receivers` (or injected
 *   into the result of this function).
 *
 *   For backwards compatibility the library also accepts a field
 *   `team_receiver_format` which follows the same rules as entries of
 *   `team_receiver_formats` and is converted into a list of length 1.
 *
...
local alertmanagerConfig(adParams, amConfig, nullReceiver=_nullR, fallbackTeam=null) =
  ...
  local finalRoute =
    local formats = teamReceiverFormats(adParams);
    if ownerOrFallbackTeam(fallbackTeam) != null then
      std.mapWithIndex(
        function(i, format) {
          receiver: format % ownerOrFallbackTeam(fallbackTeam),
          continue: i < std.length(formats) - 1,
        },
        formats
      )
    else
